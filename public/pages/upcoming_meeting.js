...
$(function () {
	...
	onDataTable_Meeting = $('#Meeting_TableId').DataTable({
		...
		"ajax": {
			"url": base_url + '/backend/getUpcomingMeetingDataLoad',
			"type": "POST",
			"data": function (data) {
				data["params"] = {
		...
		columns: [{
				...
				"orderable": false,
				"bVisible": true,
				defaultContent: "<a class='editIconBtn' title='"+TEXT['Edit']+"' href='javascript:void(0);'><i class='fa fa-edit'></i></a>"
	...
	$('#Meeting_TableId').on('click', 'tr', function (e) {
		...
		if ((className == 'editIconBtn')||(className == 'fa fa-edit')){
			...
			onCustomModal(msg, "getMeetingDetails");
		}
